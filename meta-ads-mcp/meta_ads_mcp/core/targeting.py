...
async def validate_interests(
    ...
    """
    Validate if interests are still valid for ad targeting (v25.0).

    Meta can remove interests at any time. Always validate before using in ad sets
    to avoid creation errors. Pass either interest names or IDs.

    Args:
        interest_list: List of interest names to validate (e.g., ["Marketing digital", "Fitness"])
        interest_ids: List of interest IDs to validate (e.g., ["6003127206524", "6003384241848"])
        access_token: Meta API access token (optional)

    Returns:
        JSON with validation results. Each interest returns {name, id, valid} fields.
        Invalid interests should NOT be used in targeting.
    """
    if not interest_list and not interest_ids:
        return json.dumps({"error": "Provide interest_list (names) or interest_ids"}, indent=2)
    ...
    if interest_ids:
        params["interest_fbid_list"] = json.dumps(interest_ids)
