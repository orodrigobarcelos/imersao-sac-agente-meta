...
async def make_api_request(
    ...
    Args:
        ...
        use_multipart: Use multipart/form-data instead of form-urlencoded for POST
        api_version: Override the default API version (e.g., "v24.0")
    ...
    async with httpx.AsyncClient() as client:
        try:
            if method == "GET":
                ...
                response = await client.get(url, params=encoded_params, headers=headers, timeout=60.0)
            elif method == "POST":
                # For Meta API, POST requests typically use form-urlencoded
                ...
                # Convert lists, dicts to JSON strings and booleans to '1'/'0'
                ...
                if use_multipart:
                    # Multipart/form-data (like curl -F) — required for complex JSON
                    # params like audience rules with inclusions
                    multipart_fields = {k: (None, str(v)) for k, v in request_params.items()}
                    response = await client.post(url, files=multipart_fields, headers=headers, timeout=60.0)
                else:
                    # Standard form-urlencoded (default for most Meta API endpoints)
                    response = await client.post(url, data=request_params, headers=headers, timeout=60.0)
            elif method == "DELETE":
                response = await client.delete(url, params=request_params, headers=headers, timeout=60.0)
            else:
            ...
            # Include details for debugging (sanitize URLs to avoid token leakage)
            def _sanitize_url(url: str) -> str:
                """Remove access_token from URL to prevent credential leakage."""
                import re
                return re.sub(r'access_token=[^&]+', 'access_token=***REDACTED***', str(url))
                ...
                return json.dumps({
                    "error": {
                        ...
                        "details": {
                            ...
                            "action_required": "Please provide a valid META_ACCESS_TOKEN or authenticate via OAuth",
                            "auth_url": auth_url,
