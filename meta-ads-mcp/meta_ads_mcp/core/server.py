...
class StreamableHTTPHandler:
    ...
    async def handle_request(self, request_headers: Dict[str, str], request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if auth_config['auth_method'] == 'direct_meta_token':
                return self.handle_direct_meta_token_request(auth_config, request_body)
            elif auth_config['auth_method'] == 'bearer':
    ...
    def get_auth_config_from_headers(self, request_headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        """
        # Check for direct Meta token (legacy compatibility)
        meta_token = (
            request_headers.get('X-META-ACCESS-TOKEN') or 
            request_headers.get('x-meta-access-token')
        )
        
        if meta_token:
            logger.info("Direct Meta token authentication detected (legacy)")
            return {
                'auth_method': 'direct_meta_token',
        ...
        # Fallback: Custom Meta app
        meta_app_id = request_headers.get('X-META-APP-ID') or request_headers.get('x-meta-app-id')
        if meta_app_id:
            logger.debug("Custom Meta app authentication detected (fallback)")
            return {
    ...
    def handle_direct_meta_token_request(self, auth_config: Dict[str, Any], request_body: Dict[str, Any]) -> Dict[str, Any]:
        """Handle request with direct Meta token (legacy compatibility)
        ...
        """
        logger.debug("Processing direct Meta token request (legacy)")
        meta_token = auth_config['meta_token']
        
        # Use contextvars (request-scoped) instead of os.environ (process-global)
        ...
        return {
            ...
            'result': {
                'status': 'ready',
                'auth_method': 'direct_meta_token',
                'message': 'Authentication successful with direct Meta token',
                'token_source': 'direct_header'
            },
            'id': request_body.get('id')
        }
        ...
        return {
            ...
            'error': {
                ...
                'data': {
                    ...
                    'documentation': 'Provide a valid Bearer token or Meta access token'
                }
            },
            'id': request_body.get('id')
        }
...
def main():
    ...
    if args.transport == "streamable-http":
        ...
        from . import targeting, custom_audiences, saved_audiences, custom_conversions
        from . import duplication, ab_testing
