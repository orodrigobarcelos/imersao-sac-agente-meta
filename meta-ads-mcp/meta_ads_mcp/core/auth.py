...
class AuthManager:
    ...
    def _load_cached_token(self) -> bool:
        ...
        try:
            with open(cache_path, "r") as f:
                ...
                if self.token_info.expires_in is not None:
    ...
    def invalidate_token(self) -> None:
        ...
        if self.token_info:
            logger.info(f"Invalidating token: {self.token_info.access_token[:4]}...")
            self.token_info = None
...
def exchange_token_for_long_lived(short_lived_token):
    ...
    try:
        # Check for per-request Meta App credentials (user's own app)
        from .http_auth_integration import FastMCPAuthIntegration
        ctx_app_id = FastMCPAuthIntegration.get_meta_app_id()
        ctx_app_secret = FastMCPAuthIntegration.get_meta_app_secret()

        # Use per-request credentials if available, otherwise fall back to env/config
        app_id = ctx_app_id or meta_config.get_app_id()
        app_secret = ctx_app_secret or os.environ.get("META_APP_SECRET", "")

        if ctx_app_id:
            logger.info(f"Using per-user Meta App credentials for token exchange (app_id: {ctx_app_id})")
...
async def get_current_access_token() -> Optional[str]:
    ...
    try:
        ...
        if token:
            ...
            logger.debug(f"Access token found in auth_manager (starts with: {token[:4]}...)")
            return token
        else:
...
def login():
    ...
    try:
        ...
        try:
            ...
            print("- Set META_ACCESS_TOKEN environment variable for direct token authentication")
            return
